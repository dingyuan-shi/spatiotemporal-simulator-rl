...
class Recorder:
    ...
    def update_driver_income_after_rejection(self, assignment, dispatch_observ, index2hash=None):
        """
        this function update the driver's income.
        Should be called after the rejection process.
        :param assignment: a set of ODPair, one dict is <'order_id': xxx, 'driver_id':xxx>
        ...
        if index2hash is None:
            for pair in assignment:
                self.drivers_total_income[pair.driver_id] += order_price[pair.order_id]
                self.drivers_income_per_hour[pair.driver_id][cur_hour] += order_price[pair.order_id]
                self.drivers_serving_order_info[pair.driver_id].append(order_info[pair.order_id])
        else:
            for pair in assignment:
